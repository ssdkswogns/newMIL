...
python visualize.py \
  ...
  --model newTimeMIL \
  --datatype mixed \
  --ckpt .savemodel/millet_mixed_data/ERing/exp_0/weights/best_MILLET.pth \
...
def build_testset(args):
    ...
    # ---- AEON ----
    train_ret = load_classification(name=args.dataset, split='train', extract_path='./data')
    if isinstance(train_ret, (list, tuple)) and len(train_ret) == 3:
        _, _, meta = train_ret
    else:
        meta = {}

    test_ret = load_classification(name=args.dataset, split='test', extract_path='./data')
    if isinstance(test_ret, (list, tuple)) and len(test_ret) >= 2:
        Xte, yte = test_ret[:2]
    else:
        raise ValueError("load_classification(test) must return at least (X, y).")
    ...
    elif yte_np.dtype.kind in ('U', 'S', 'O'):  # string-ish labels
        cls_vals, inv = np.unique(yte_np, return_inverse=True)
        class_names = list(cls_vals)
        yte_idx = torch.tensor(inv, dtype=torch.long)
    else:
...
def infer_pred_inst(args, model, x, y_inst_onehot):
    ...
    if args.model == "MILLET":
        ...
        if not isinstance(out, (tuple, list)) or len(out) < 2:
            raise ValueError("MILLET forward must return (bag_logits, instance_pred, [interpretation]).")

        instance_pred = out[1]
        interpretation = out[2] if len(out) >= 3 else None

        if instance_pred is None:
            raise ValueError("MILLET forward did not return instance predictions. Ensure is_instance=True.")

        attn_ct = None
        pred_source = instance_pred
        pred_dim = 2  # instance_pred shape: [B, T, C]

        if interpretation is not None and interpretation.dim() == 3:
            if interpretation.shape[1] == C_gt:
                attn_ct = interpretation  # [B, C, T]
                pred_source = attn_ct
                pred_dim = 1
            elif interpretation.shape[2] == C_gt:
                attn_ct = interpretation.transpose(1, 2)  # [B, C, T]
                pred_source = attn_ct
                pred_dim = 1

        pred_inst = torch.argmax(pred_source, dim=pred_dim)  # [B, T]
...
def main():
    ...
    parser.add_argument('--model', default='newTimeMIL', type=str, choices=['TimeMIL', 'newTimeMIL', 'AmbiguousMIL', 'MILLET'])
