...
def instance_prototype_contrastive_loss(
    ...
    # similarity: S[b, t, c] = <z_{b,t}, p_c> / tau
    ...
    for b in range(B):
        ...
        if len(anchors_b) == 0:
            print("No valid anchor in threshold")
            # 각 (b,t)에 대해 valid class 중 argmax를 anchor로, confidence는 그 sim
            for b in range(B):
                for t in range(T):
                    sims_bt = S_valid[b, t]        # [C]
                    max_sim, c_star = sims_bt.max(dim=-1)
                    if max_sim <= -1e8:
                        continue  # valid class 없음
                    anchors_b.append(b)
                    anchors_t.append(t)
                    anchors_c.append(c_star)
                    anchor_conf.append(max_sim.item())

        if len(anchors_b) == 0:
            # 진짜로 valid class가 하나도 없는 경우 (이론적으로 거의 없음)
            return torch.tensor(0.0, device=device)
    ...
    loss = -(pos_sim - log_all).mean()
    return loss

# def multi_context_contrastive_loss(x_seq, c_seq, tau=0.1, win_size=2):
#     """
#     x_seq : [B, T, D]  -> per-timestep feature z_t
#     c_seq : [B, T, D]  -> local-context feature c_t (overlapping window 기반)
#     tau   : temperature
#     win_size : positive 로 볼 시간 거리 |t - t'| <= win_size

#     한 시퀀스(b)에 대해:
#       - anchor: z_t
#       - positives: { c_{t'} | |t - t'| <= win_size }
#       - negatives: 같은 시퀀스 내의 나머지 c_{t'}
#     를 사용하는 multi-positive InfoNCE.
#     """
#     B, T, D = x_seq.shape
#     device = x_seq.device
...
#     x_norm = F.normalize(x_seq, dim=-1)           # [B, T, D]
#     c_norm = F.normalize(c_seq, dim=-1)  # [B, T, D], teacher 쪽은 stop-grad
...
#     # |i - j| matrix
#     diff = (idx[None, :] - idx[:, None]).abs()  # [T, T]
#     pos_mask_base = (diff <= win_size)          # [T, T] (True = positive)
...
#         # similarity: [T, T]
#         sim = torch.matmul(z, c.transpose(0, 1)) / tau

#         # positive mask: (복사해서 사용)
#         pos_mask = pos_mask_base  # [T, T]

#         # logsumexp over positives / all
#         sim_pos = sim.masked_fill(~pos_mask, -1e9)   # positives만 남김
#         log_pos = torch.logsumexp(sim_pos, dim=1)    # [T]
#         log_all = torch.logsumexp(sim, dim=1)        # [T]

#         loss_b = -(log_pos - log_all).mean()
#         total_loss += loss_b
#         count += 1
...
def multi_context_contrastive_loss(x_seq, c_seq, anchor_weight=None, tau=0.1, win_size=2):
    """
    x_seq : [B, T, D]  -> per-timestep feature z_t
    c_seq : [B, T, D]  -> local-context feature c_t
    anchor_weight : [B, T] or None
        - 각 timestep마다 CL loss 가중치 (p_inst 기반으로 계산)
        - None이면 모든 timestep을 동일하게 평균
    ...
    x_norm = F.normalize(x_seq, dim=-1)           # [B, T, D]
    c_norm = F.normalize(c_seq, dim=-1)           # [B, T, D]
    ...
    idx = torch.arange(T, device=device)          # [T]
    diff = (idx[None, :] - idx[:, None]).abs()    # [T, T]
    pos_mask_base = (diff <= win_size)            # [T, T]

    for b in range(B):
        z = x_norm[b]      # [T, D]
        c = c_norm[b]      # [T, D]
        sim = torch.matmul(z, c.transpose(0, 1)) / tau    # [T, T]
        ...
        per_timestep_loss = -(log_pos - log_all)     # [T]

        if anchor_weight is not None:
            w = anchor_weight[b]                     # [T]
            # 안정성을 위해 합으로 나눔 (0 division 방지)
            loss_b = (per_timestep_loss * w).sum() / (w.sum() + 1e-6)
        else:
            loss_b = per_timestep_loss.mean()
...
#         ctx_contrast_loss = torch.tensor(0.0, device=bag_feats.device)
...
#             with torch.no_grad():
#                 # 1) timestep confidence: max p_inst
#                 p_inst_detach = p_inst.detach()          # [B, T, C]
#                 p_max, _ = p_inst_detach.max(dim=-1)     # [B, T]

#                 # 2) timestep 변화량 (transition 근처 down-weight)
#                 diff_prob = (p_inst_detach[:, 1:, :] - p_inst_detach[:, :-1, :]).abs().mean(dim=-1)  # [B, T-1]

#                 boundary = torch.ones_like(p_max)        # [B, T]
#                 boundary[:, 1:] = torch.exp(-diff_prob)  # 변화 클수록 ↓

#                 # 3) confidence * boundary → soft weight
#                 anchor_weight = (p_max * boundary).clamp(0.3, 1.0)  # [B, T], 완전 0/1 말고 soft

#             ctx_contrast_loss = multi_context_contrastive_loss(
#                 x_seq, c_seq,
#                 anchor_weight=anchor_weight,
#                 tau=getattr(args, "ctx_tau", 0.1),
#                 win_size=getattr(args, "ctx_win", 2),
#             )

#         if args.model == 'AmbiguousMIL':
#             warm = max(args.epoch_des, 1)
#             if epoch <= warm:
#                 ctx_w = 0.0
#             else:
#                 ratio = min((epoch - warm) / float(warm), 1.0)  # warm epoch 동안 0→1
#                 ctx_w = args.ctx_contrast_w * ratio
#         else:
#             ctx_w = 0.0
...
#         sum_ctx_contrast += ctx_contrast_loss.item()
#         sum_total += loss.item()
#         n += 1
...
def train(trainloader, milnet, criterion, optimizer, epoch, args, proto_bank=None):
    milnet.train()
    total_loss = 0
    sum_bag = 0.0
    sum_inst = 0.0
    ...
    for batch_id, (feats, label) in enumerate(trainloader):
        ...
        sparsity_loss = torch.tensor(0.0, device=bag_feats.device)
        # ctx_contrast_loss = torch.tensor(0.0, device=bag_feats.device)
        proto_inst_loss = torch.tensor(0.0, device=bag_feats.device)
        ...
        if args.model == 'AmbiguousMIL':
            loss = (
                ...
                + args.smooth_loss_w * smooth_loss
                + args.sparsity_loss_w * sparsity_loss
                # + args.ctx_contrast_w * ctx_contrast_loss
                + args.proto_loss_w * proto_inst_loss      # ★ 새로 추가된 항목
            )
            sys.stdout.write(
                '\r Training bag [%d/%d] bag loss: %.4f  total loss: %.4f' %
                (batch_id, len(trainloader), bag_loss.item(), loss.item())
...
# def test(testloader, milnet, criterion, epoch, args, threshold: float = 0.5, proto_bank=None):
#     milnet.eval()
#     total_loss = 0.0
#     all_labels = []
#     all_probs  = []
...
#                 smooth_loss = (diff ** 2).mean()
#                 with torch.no_grad():
#                     p_inst_detach = p_inst.detach()
#                     p_max, _ = p_inst_detach.max(dim=-1)
#                     diff_prob = (p_inst_detach[:, 1:, :] - p_inst_detach[:, :-1, :]).abs().mean(dim=-1)
#                     boundary = torch.ones_like(p_max)
#                     boundary[:, 1:] = torch.exp(-diff_prob)
#                     anchor_weight = (p_max * boundary).clamp(0.3, 1.0)
...
def test(testloader, milnet, criterion, epoch, args, threshold: float = 0.5, proto_bank=None):
    ...
    with torch.no_grad():
        for batch_id, (feats, label) in enumerate(testloader):
            ...
            smooth_loss = torch.tensor(0.0, device=bag_feats.device)
            sparsity_loss = torch.tensor(0.0, device=bag_feats.device)
            # ctx_contrast_loss = torch.tensor(0.0, device=bag_feats.device)  # [NEW]
            proto_inst_loss = torch.tensor(0.0, device=bag_feats.device)    # [NEW]
            ...
            if instance_pred is not None:
                ...
                # temporal smoothness loss: (p_t - p_{t-1})^2, positive class만
                diff = p_inst[:, 1:, :] - p_inst[:, :-1, :]   # [B, T-1, C]
                diff = diff * pos_mask[None, None, :]         # broadcast
                smooth_loss = (diff ** 2).mean()

                # ---- context CL용 anchor weight (p_inst 기반) ----
                # with torch.no_grad():
                #     p_inst_detach = p_inst.detach()
                #     p_max, _ = p_inst_detach.max(dim=-1)                   # [B, T]
                #     diff_prob = (p_inst_detach[:, 1:, :] - p_inst_detach[:, :-1, :]).abs().mean(dim=-1)  # [B, T-1]
                #     boundary = torch.ones_like(p_max)                       # [B, T]
                #     boundary[:, 1:] = torch.exp(-diff_prob)                # transition 부근 down-weight
                #     anchor_weight = (p_max * boundary).clamp(0.3, 1.0)     # [B, T]
            ...
            # ctx_contrast warmup 스케줄 (train과 동일 로직)
            if args.model == 'AmbiguousMIL':
                warm = max(args.epoch_des, 1)
                if epoch <= warm:
                    ctx_w = 0.0
                else:
                    ratio = min((epoch - warm) / float(warm), 1.0)
                    ctx_w = args.ctx_contrast_w * ratio
            else:
                ctx_w = 0.0
            ...
            if args.model == 'AmbiguousMIL':
                loss = (
                    ...
                    + args.sparsity_loss_w * sparsity_loss
                    # + ctx_w * ctx_contrast_loss
                    + args.proto_loss_w * proto_inst_loss     # [NEW]
                )
                sys.stdout.write(
                    '\r Testing bag [%d/%d] bag loss: %.4f  total loss: %.4f' %
                    (batch_id, len(testloader), bag_loss.item(), loss.item())
    ...
    wandb.log({
        ...
        "val/sparsity_loss": sum_sparsity / max(1, n),
        "val/ctx_contrast_loss": sum_ctx_contrast / max(1, n),
        "val/proto_inst_loss": sum_proto_inst / max(1, n),   # [NEW]
        "val/total_loss": sum_total / max(1, n),
    }, step=epoch)
...
def main():
    ...
    args.ortho_loss_w = 0
    args.smooth_loss_w = 0.05
    args.sparsity_loss_w = 0.05
    args.ctx_contrast_w = 0
    args.proto_loss_w = 0.05
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        train_loss_bag = train(trainloader, milnet, criterion, optimizer, epoch, args, proto_bank=proto_bank)      
        test_loss_bag, results = test(testloader, milnet, criterion, epoch, args, threshold=0.5, proto_bank=proto_bank)
        wandb.log({
            "epoch": epoch,
