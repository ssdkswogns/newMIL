...
class TSNERecorder:
    """
    Epoch마다 instance 임베딩과 prototype을 모아서
    - raw 임베딩(.npz) 저장
    - 옵션에 따라 epoch마다 바로 t-SNE까지 수행해 저장
    """
    def __init__(
        self,
        save_dir: str,
        ...
        self.base_dir = os.path.join(save_dir, "tsne")
        self.logging = False
        self.current_epoch = None
        self.instances = []
        self.inst_pred = []
        self.proto_embed = None
        self.proto_label = None

    def start_epoch(self, epoch: int, split: str = "val"):
        if not self.enabled:
            return False
        self.logging = (epoch % self.every == 0)
        if not self.logging:
            return False
    ...
    def add_batch(self, x_seq: torch.Tensor, instance_pred: torch.Tensor = None):
        if not self.logging:
            return
        with torch.no_grad():
            B, T, D = x_seq.shape
            flat_feat = x_seq.detach().cpu().reshape(-1, D)  # [B*T, D]

            pred_cls = None
            if instance_pred is not None:
                pred_cls = torch.argmax(instance_pred.detach(), dim=-1).reshape(-1).cpu()  # [B*T]

            num_left = self.max_instances - sum(x.size(0) for x in self.instances)
            if num_left <= 0:
                return

            if flat_feat.size(0) > num_left:
                idx = torch.randperm(flat_feat.size(0))[:num_left]
                flat_feat = flat_feat[idx]
                if pred_cls is not None:
                    pred_cls = pred_cls[idx]

            self.instances.append(flat_feat)
            if pred_cls is not None:
                self.inst_pred.append(pred_cls)

    def set_prototypes(self, prototypes: torch.Tensor, labels: torch.Tensor):
        if not self.logging:
            return
        self.proto_embed = prototypes.detach().cpu()
        self.proto_label = labels.detach().cpu()

    def _run_tsne(self, data: np.ndarray):
        if data.shape[0] <= 2:
            return None
        perp = min(self.perplexity, max(1, data.shape[0] - 1))
        tsne = TSNE(
            n_components=2,
            perplexity=perp,
            n_iter=self.n_iter,
    ...
    def finish_epoch(self, epoch: int):
        ...
        inst = torch.cat(self.instances, dim=0) if self.instances else torch.empty(0, device="cpu")
        pred = torch.cat(self.inst_pred, dim=0) if self.inst_pred else torch.empty(0, device="cpu", dtype=torch.long)

        save_base = os.path.join(self.base_dir, f"epoch_{epoch:04d}")

        proto_np = self.proto_embed.numpy() if self.proto_embed is not None else np.array([])
        proto_label_np = self.proto_label.numpy() if self.proto_label is not None else np.array([])

        np.savez(
            f"{save_base}_raw.npz",
            inst_embed=inst.numpy(),
            inst_pred=pred.numpy(),
            proto_embed=proto_np,
            proto_label=proto_label_np,
            split=self.split,
        )

        if self.compute_now:
            combined = inst
            labels = pred.numpy() if pred.numel() > 0 else np.array([])
            kinds = np.zeros(combined.size(0), dtype=np.int64)

            if self.proto_embed is not None and self.proto_embed.numel() > 0:
                combined = torch.cat([combined, self.proto_embed], dim=0)
                proto_kind = np.ones(self.proto_embed.size(0), dtype=np.int64)
                kinds = np.concatenate([kinds, proto_kind])
                proto_labels = proto_label_np
                labels = np.concatenate([labels, proto_labels])

            coords = self._run_tsne(combined.numpy())
            if coords is not None:
                np.savez(
                    f"{save_base}_tsne.npz",
                    coords=coords,
                    label=labels,
                    kind=kinds,
                    split=self.split,
...
def test(testloader, milnet, criterion, epoch, args, device, threshold: float = 0.5, proto_bank=None, is_main=True, tsne_recorder: TSNERecorder = None):
    milnet.eval()
    total_loss = 0.0
    all_labels = []
    all_probs  = []
    ...
    if tsne_recorder is not None:
        tsne_recorder.start_epoch(epoch, split="val")
    ...
    with torch.no_grad():
        for batch_id, batch in enumerate(testloader):
            ...
            if instance_pred is not None:
                ...
                if tsne_recorder is not None and tsne_recorder.logging and x_seq is not None:
                    tsne_recorder.add_batch(x_seq, instance_pred)
    ...
    if tsne_recorder is not None and tsne_recorder.logging:
        if proto_bank is not None:
            proto, initialized = proto_bank.get()
            if initialized is not None:
                ...
                mask = norm > 1e-6
            if mask.any():
                cls_idx = torch.nonzero(mask, as_tuple=False).squeeze(-1)
                tsne_recorder.set_prototypes(proto[cls_idx], cls_idx)
        tsne_recorder.finish_epoch(epoch)
...
def main():
    ...
    parser.add_argument('--label_threshold', type=float, default=0.5,
                        help='Threshold for selecting positive classes when computing AOPCR')
    parser.add_argument('--tsne_every', type=int, default=0,
                        help='Save t-SNE-ready embeddings every N epochs (0 to disable)')
    parser.add_argument('--tsne_max_instances', type=int, default=2000,
                        help='Max number of instance embeddings to keep per logged epoch')
    parser.add_argument('--tsne_compute_now', action='store_true',
                        help='If set, run t-SNE immediately each logging epoch and save 2D coords')
    parser.add_argument('--tsne_perplexity', type=float, default=30.0,
                        help='Perplexity for on-the-fly t-SNE (ignored if tsne_compute_now is False)')
    parser.add_argument('--tsne_n_iter', type=int, default=1000,
                        help='Iteration count for on-the-fly t-SNE')
    # loss weights
    ...
    tsne_recorder = None
    if is_main and args.tsne_every > 0:
        tsne_recorder = TSNERecorder(
            save_dir=args.save_dir,
            every=args.tsne_every,
            max_instances=args.tsne_max_instances,
            compute_now=args.tsne_compute_now,
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        if is_main:
            test_loss_bag, results = test(testloader, milnet, criterion, epoch, args, device, threshold=0.5, proto_bank=proto_bank, is_main=is_main, tsne_recorder=tsne_recorder)
    ...
    if is_main and best_model_path is not None:
        ...
        if eval_model is not None:
            ...
            aopcr_c, aopcr_w_avg, aopcr_mean, M_expl, M_rand, alphas, counts = compute_classwise_aopcr(
                ...
                step=0.05,
                n_random=3,
                label_threshold=getattr(args, "label_threshold", 0.5),
                fallback_to_predicted=True,
            )
