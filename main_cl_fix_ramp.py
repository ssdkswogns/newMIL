...
def instance_prototype_contrastive_loss(
    ...
    # ----- Optional: class별 top-k anchor로 안정화 -----
    if topk_ratio > 0.0:
        # 후보 (b,t,c) 점수 행렬: 직접/이웃 최대 유사도만 사용
        # scores[b,t] = max_c valid S_valid[b,t,c]
        scores = S_tmax
        anchors_mask = torch.zeros_like(anchors_mask, dtype=torch.bool)
        # per class 선택
        for c in range(C):
            # 해당 클래스가 등장한 bag만 고려
            valid_mask = (bag_label[:, c] > 0).view(B, 1).expand(B, T)
            sim_c = S_valid[:, :, c]
            sim_c = sim_c.masked_fill(~valid_mask, -1e9)
            flat = sim_c.view(-1)
            # topk 개수
            k = max(topk_min, int(flat.numel() * topk_ratio))
            if k <= 0:
                continue
            topk_vals, topk_idx = torch.topk(flat, k)
            topk_mask = torch.zeros_like(flat, dtype=torch.bool)
            topk_mask[topk_idx] = True
            anchors_mask = anchors_mask | topk_mask.view(B, T)
            c_all = torch.where(topk_mask.view(B, T), torch.full_like(c_all, c), c_all)
    ...
    with torch.no_grad():
        ...
        weights_per_class = torch.zeros(C, device=device)
        valid = counts > 0
        # sqrt + clip to avoid explosion
        weights_per_class[valid] = torch.sqrt(counts.sum() / counts[valid])
        if w_clip is not None and w_clip > 0:
            weights_per_class = torch.clamp(weights_per_class, max=w_clip)
...
def train(trainloader, milnet, criterion, optimizer, epoch, args, device, proto_bank=None, is_main=True, world_size=1):
    ...
    for batch_id, (feats, label) in enumerate(trainloader):
        ...
        ramp_start = getattr(args, "proto_ramp_start", 0)
        ramp_len = getattr(args, "proto_ramp_len", 0)
        ramp_lam = 1.0
        if ramp_len > 0:
            ramp_lam = max(0.0, min(1.0, float(epoch - ramp_start) / float(ramp_len)))
        ...
        if instance_pred is not None:
            ...
            if (epoch >= args.epoch_des) and (proto_bank is not None):
                # prototype update after ramp window
                if epoch >= ramp_start + ramp_len:
                ...
                proto_inst_loss = instance_prototype_contrastive_loss(
                    ...
                    topk_ratio=getattr(args, "proto_topk_ratio", 0.0),
                    topk_min=getattr(args, "proto_topk_min", 0),
                    w_clip=getattr(args, "proto_w_clip", 10.0),
                )
                proto_inst_loss = proto_inst_loss * ramp_lam
...
def main():
    ...
    parser.add_argument('--proto_topk_ratio', type=float, default=0.0,
                        help='if >0, use per-class top-k ratio for anchors (e.g., 0.1)')
    parser.add_argument('--proto_topk_min', type=int, default=0,
                        help='minimum anchors per class when using top-k')
    parser.add_argument('--proto_w_clip', type=float, default=10.0,
                        help='clip upper bound for class-balanced weights (sqrt form)')
    parser.add_argument('--proto_ramp_start', type=int, default=20,
                        help='epoch to start ramping prototype loss/updates')
    parser.add_argument('--proto_ramp_len', type=int, default=20,
                        help='ramp length (epochs) for prototype loss/updates')
    # loss weights
