...
from models.nystrom_attention import NystromAttention, Attention, Attention_ours
...
class TransLayer(nn.Module):
    def __init__(self, norm_layer=nn.LayerNorm, dropout=0.2,dim=512, num_heads=8):
        super().__init__()
        ...
        # For constancy of attention length with sequence length, we use regular attention here.s
        self.attn = Attention(
            dim=dim,
            num_heads=num_heads,
            qkv_bias=True,
...
class AmbiguousMIL_v2(nn.Module):
    """
    AmbiguousMIL v2: Transformer + MILLET-style aggregation

    Architecture:
    - Feature Extractor (InceptionTime)
    - Wavelet PE + Transformer layers (context-aware representation)
    - MILLET-style instance aggregation:
      * attn = σ(Linear(Tanh(Linear(x_seq))))
      * instance_logits = Linear(x_seq)
      * bag_logits = mean(instance_logits * attn)
    """
    def __init__(self, in_features, n_classes=2, mDim=64, max_seq_len=400, dropout=0., is_instance=False):
        super().__init__()
        self.is_instance = is_instance
        ...
        # Wavelet Positional Encoding parameters
        self.wave1 = torch.randn(2, mDim, 1)
        ...
        # MILLET-style instance aggregation
        # Attention head: mDim -> 8 -> 1
        self.inst_attn = nn.Sequential(
            nn.Linear(mDim, 8),
            nn.Tanh(),
            nn.Linear(8, 1),
            nn.Sigmoid()
        ...
        # Instance classifier: 1-layer Linear (simplified)
        self.instance_head = nn.Linear(mDim, n_classes)
    ...
    def forward(self, x, warmup=False):
        ...
        # Wavelet PE + Transformer Layer 1
        x = self.pos_layer(x, self.wave1, self.wave2, self.wave3, self.n_classes)
        x, attn_layer1 = self.layer1(x)

        # Wavelet PE + Transformer Layer 2
        x = self.pos_layer2(x, self.wave1_, self.wave2_, self.wave3_, self.n_classes)
        x, attn_layer2 = self.layer2(x)

        # Extract sequence tokens (context-aware from Transformer)
        x_cls = x[:, :C, :]       # [B, C, D]
        x_seq = x[:, C:, :]       # [B, T, D]

        # MILLET-style instance aggregation
        attn = self.inst_attn(x_seq)                      # [B, T, 1]
        instance_logits = self.instance_head(x_seq)       # [B, T, C]
        weighted_instance_logits = instance_logits * attn # [B, T, C]
        bag_logits = weighted_instance_logits.mean(dim=1) # [B, C]

        if self.is_instance:
            # Return attention for visualization
            return bag_logits, instance_logits, x_cls, x_seq, attn_layer1, attn_layer2, attn
        else:
